(function(){
  ...
  try{
    ...
    const PadMode = Object.freeze({
      ...
      RECORD: 'record' // NEW: pad-level mic one-shot recorder
    });
    ...
    // helper to render a vslider cell
    ...
    function rebuildKitEditor(){
      ...
      for (let i=0;i<PAD_COUNT;i++){
        ...
        // Row-level input handling
        row.addEventListener('input', async (e)=>{
          const t = e.target;
          const k = t.dataset.k;
          if (!k) return;

          // Visual value readout for sliders
    ...
    /* --- TOGGLE (start/stop with optional resume) --- */
    ...
    /* === Pad RECORD mode (press to record one-shot, press again to stop → assign) === */
    let micStream = null;
    const padRecorders = new Map(); // idx -> {rec, chunks:[], active:true}

    // Arm mic permission on first gesture anywhere
      ...
      try{
        ...
        rec.onstop = async ()=>{
          ...
          try{
            ...
            pads[idx].toggleOn=false; pads[idx].savedOffset=0; pads[idx].voice=null;
            buildGrid(); // refresh pad labels/rows
    ...
    // Sequencer inputs + BPM number field sync
    seqBPMInput.oninput = ()=>{
    ...
    /* ===== Local Kit Manager (persists seq too) ===== */
        ...
        if (!micRecOS || micRecOS.state === 'inactive'){
          ...
          micRecOS.onstop = async ()=>{
            try{
              ...
              status('One-shot recorded → keyboard ready');
    ...
    /* ===== Sampler Piano (2 octaves + shift, one-shot/gate) ===== */
    const BLACKS = new Set([1,3,6,8,10]);
    let pianoBase = 48; // C3
    let sampOneShot = true; // default one-shot
    const pianoVoices = new Map(); // midi -> {src}
    ...
    function currentOctLabel(){
      ...
      return midiNoteName(left) + '–' + midiNoteName(right);
    }
    function clearPiano(){
    ...
    function attachKeyHandlers(el, midi){
      function start(){
        if (!sampBuf) return;
        const {start:startSec, end:endSec} = regionForPlay();
        const src = ctx.createBufferSource();
        src.buffer = sampBuf;
        const g = ctx.createGain();
        g.gain.value = 1;
